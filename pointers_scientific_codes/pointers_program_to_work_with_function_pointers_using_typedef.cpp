...
// Программа для работы с указателями на функции с использованием typedef:

typedef int (*ArithmeticFunction)(int, int); // Определение типа указателя на функцию
...
int main() {
    ...
    ArithmeticFunction ptr; // Использование typedef для объявления указателя
